...
export const useEvents = () => {
  const events = ref<Event[]>([
      ...
      reviews: [
        {
          ...
          userName: 'Sarah M.',
          rating: 5,
          comment: 'Amazing night! The acoustics were perfect and the atmosphere was incredible.',
          date: '2025-05-15',
          helpful: 12
        },
        {
    ...
    },
    {
      ...
      organizer: {
        name: 'The Pearson Pub Events Team',
        description: 'Creating memorable experiences and bringing the community together'
      },
      reviews: [
        {
          id: 'r4',
          ...
          rating: 5,
          comment: 'Perfect family event! Kids loved it and the food was incredible.',
          date: '2024-07-20',
          helpful: 25
        }
      ],
  ...
  const weeklySchedule = ref<WeeklySchedule[]>([
    ...
    // },
    {
      ...
      icon: 'i-heroicons-musical-note',
      description: 'Live bands and high-energy performances to kick off the weekend.',
      recurring: true,
      price: 0,
      link: '/events'
    },
  ...
  const featuredEvents = computed(() =>
    events.value.filter(event => event.featured && event.status === 'upcoming')
  )
  ...
  const searchEvents = (query: string) => {
    ...
    return events.value.filter(
      event =>
        ...
        event.performers?.some(performer => performer.toLowerCase().includes(lowercaseQuery))
    )
  }
