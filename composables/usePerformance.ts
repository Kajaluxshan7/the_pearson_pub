...
export const usePerformance = () => {
  ...
  const startFPSMonitoring = () => {
    ...
    const countFrame = () => {
      ...
      if (currentTime >= startTime + 1000) {
        metrics.value.fps = Math.round((frames * 1000) / (currentTime - startTime))
        frames = 0
  ...
  const monitorResourceLoading = () => {
    ...
    const updateResourceMetrics = () => {
      ...
      metrics.value.resourcesLoaded = resources.filter(r => r.duration > 0).length
    }
  ...
  const loadElementContent = async (element: HTMLElement, config: LazyLoadConfig) => {
  ...
  const applyImageSrc = (img: HTMLImageElement, src: string, isPlaceholder = false) => {
  ...
  const measureCoreWebVitals = () => {
    ...
    new PerformanceObserver(() => {
      // LCP tracked for monitoring
    }).observe({ type: 'largest-contentful-paint', buffered: true })

    // First Input Delay (FID)
    new PerformanceObserver(() => {
      // FID tracked for monitoring
