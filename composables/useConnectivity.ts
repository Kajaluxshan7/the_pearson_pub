...
export const useConnectivity = () => {
  ...
  const checkBackendHealth = async (timeout = 8000): Promise<boolean> => {
    ...
    try {
      ...
      if (process.dev && error instanceof Error && error.name !== 'AbortError') {
  ...
  // Computed properties
  const canUseBackend = computed(() => status.value.isOnline && status.value.backendReachable)
  ...
  const connectionQuality = computed(() => {
    ...
    if (!status.value.backendReachable) {
      return 'limited'
    }
    return 'good'
  })
