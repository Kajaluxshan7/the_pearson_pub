<template>
  <section class="py-16 lg:py-20 bg-white dark:bg-gray-800 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      <div class="text-center mb-12 lg:mb-16">
        <h2 class="text-3xl lg:text-5xl font-bold text-gray-900 dark:text-white mb-4" style="font-family: 'Cinzel', 'Georgia', serif;">
        ...
        <p class="text-lg lg:text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        >
          ...
          <h3 class="text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-2 group-hover:text-yellow-600 dark:group-hover:text-yellow-400 transition-colors">
            {{ schedule.day }}
          </h3>
          
          <p class="text-yellow-600 dark:text-yellow-400 font-semibold text-lg mb-3">
            {{ schedule.event }}
          </p>
          
          <div class="flex items-center justify-center gap-2 text-gray-600 dark:text-gray-300 mb-4">
            <UIcon name="i-heroicons-clock" class="w-4 h-4" />
            <span class="text-sm font-medium">{{ schedule.time }}</span>
          </div>
          
          <p v-if="schedule.description" class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed mb-4">
          ...
          <div v-if="schedule.price || schedule.specialNote" class="space-y-2">
            <div v-if="schedule.price" class="text-sm">
              ...
              <span class="font-semibold text-yellow-600 dark:text-yellow-400 ml-1">${{ schedule.price }}</span>
            </div>
            
            <div v-if="schedule.specialNote" class="text-xs text-gray-500 dark:text-gray-400 italic">
      ...
      <div class="mt-12 lg:mt-16 text-center">
        <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-6 lg:p-8 max-w-4xl mx-auto border border-yellow-200 dark:border-yellow-800">
          <UIcon name="i-heroicons-megaphone" class="w-8 h-8 text-yellow-600 dark:text-yellow-400 mx-auto mb-4" />
